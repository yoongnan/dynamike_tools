...
@Component({
  ...
  styleUrls: ['./invoicetypelisting.component.scss']
})
export class InvoiceTypeListingComponent implements OnInit {
  ...
  addItems(id,description){
    ...
    let promise = new Promise((resolve, reject) => {
      this.dcrService.addInvoiceType(id,description)
        .toPromise()
        .then(
          res => { // Success
  ...
  update(value){    
    if(document.getElementsByClassName("trDisplayLabel"+value)[0].getAttribute("style")=="display:none"){
      let promise = new Promise((resolve, reject) => {
        this.dcrService.addInvoiceType(this.InvoiceTypes[value].id,this.InvoiceTypes[value].description)
          .toPromise()
          .then(
            res => { // Success
              this.common.createModalMessage("Successful","save successful!!!").success();
